...
export const PromptComposer: React.FC = () => {
  ...
  const tools = [
    ...
    { id: 'mask', icon: MousePointer, label: 'Select', description: 'Paint a mask' },
  ] as const;
  ...
  return (
    <>
      <div className="w-80 lg:w-72 xl:w-80 h-full bg-gray-950 border-r border-gray-800 p-6 flex flex-col space-y-6 overflow-y-auto">
        {/* AI 渠道选择 */}
        <div>
          <label className="text-sm font-medium text-gray-300 mb-2 block">AI Channel</label>
          <select
            value={aiProvider}
        ...
        <div>
          ...
          {selectedTool === 'mask' && (
            <p className="text-xs text-gray-400 mb-3">Upload an image, then paint the area you want to edit.</p>
          )}
          {selectedTool === 'generate' && (
          ...
          {selectedTool === 'edit' && (
            <p className="text-xs text-gray-500 mb-3">
              {canvasImage
                ? 'Current image is ready. You can add up to 2 style references.'
                : 'Upload the base image you want to edit.'}
            </p>
          )}
